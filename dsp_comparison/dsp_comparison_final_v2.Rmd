...
IMPORTANT NOTE: The STRING text mining data with PMIDs is from an older version of STRING< since it was obtained by Felix Langer, from Evangelia Petsalaki's group. This was done in Feb 2019 and the version of STRING and the interactions hosted there are VERY DIFFERENT from the ones we generated in Feb 2020. 
```{r STRING_TM}
...
pubpaircomp_table_check_pt1.1 <- pubpaircomp_table_check_pt1[, c("prot1", "prot2") := tstrsplit(pair_id, "_", fixed=TRUE)]
pubpaircomp_table_check_pt1.1.1 <- pubpaircomp_table_check_pt1.1[,prot2:=NULL]
colnames(pubpaircomp_table_check_pt1.1.1) <- c("pair_id","pmid","imex","reactome","tm_epmc","tm_pr_times_found","tm_dm_times_found","tm_method_term","EVEX","evex_score","BioGRID","GO_IPI","OmniPath_interactions","OmniPath_ptm","iid_pred","STRING_pi_score_ave","STRING_pi","STRING_tm_score_ave","STRING_textmining","STRING_phylo_score_ave","STRING_phylo","eval","prots")

pubpaircomp_table_check_pt1.1 <- pubpaircomp_table_check_pt1[, c("prot1", "prot2") := tstrsplit(pair_id, "_", fixed=TRUE)]
pubpaircomp_table_check_pt1.1.2 <- pubpaircomp_table_check_pt1.1[,prot1:=NULL]
colnames(pubpaircomp_table_check_pt1.1.2) <- c("pair_id","pmid","imex","reactome","tm_epmc","tm_pr_times_found","tm_dm_times_found","tm_method_term","EVEX","evex_score","BioGRID","GO_IPI","OmniPath_interactions","OmniPath_ptm","iid_pred","STRING_pi_score_ave","STRING_pi","STRING_tm_score_ave","STRING_textmining","STRING_phylo_score_ave","STRING_phylo","eval","prots")
...
# pubpaircomp_table_check_pt4 <- pubpaircomp_table_form[pair_id %in% pubpaircomp_noncur_pairs$pair_id]$noncur_prot <- "1" 
# 
