---
title: Comparison of protein association datasets and their enrichment in IMEx curated
  interactions
author: "Pablo Porras"
date: '2016-12-19'
output:
...
x =grid.text(plotname, x = unit(1.6, "npc"),y= unit(0.5, "npc"))
popViewport()
...
```{r tm_1, fig.width = 8, fig.height=3, echo=F}
{ 
grid.newpage()
...
# plot names
pushViewport(viewport(layout.pos.col=2, layout.pos.row = 1))
x =grid.text(plotname, x = unit(1.3, "npc"),y= unit(0.5, "npc"))
popViewport()
...
```{r Predictions_1, fig.width = 8, fig.height=3, echo=F}
{ 
grid.newpage()
...
```{r Interactions_1, fig.width = 8, fig.height=3, echo=F}
...
# resources enriched in the publications which contain interaction data
...
## tm_epmc is depleted in interaction-containing papers as compared to all genes-associated (by NCBI) papers!
...
qplot(x = colnames(resources_Npmids2), y =resources_Npmids2[3,], geom = "point", ylab = "fraction of interacting pairs in BioGRID", xlab = NULL) + scale_y_continuous(breaks = seq(0,1,0.05))+theme(axis.text.x = element_text(angle =-25, vjust = 0.6))+geom_text(label = paste0(signif(resources_Npmids2[2,],3)," /\n",signif(resources_Npmids2[1,],3)), vjust = -0.5)+geom_text(label = signif(resources_Npmids2[3,],3), vjust = 1.4)
```
