...
output: 
  html_document: 
    keep_md: yes
---
GO IPI, Gene Ontology - Inferred from Physical Interaction - dataset generator -wrong
========================================================
...
```{r pairs, results='hide', message=FALSE, warning=FALSE}
# selecting necessary columns
GO_IPI_pairs = GO_IPI[,.(ida = V2, idb = V8, pubid = V6, GO_IPI = 1, taxon = V13)]
...
GO_IPI_pairs = GO_IPI_pairs[, data.table(ida = ida, idbz, pubid = pubid, taxon = taxon, GO_IPI = GO_IPI)]
...
GO_IPI_pairs[, pair_id_clean := apply(data.table(ida_clean,idb_clean,stringsAsFactors = F), 1,
                                               function(a) { z = sort(a)
                                               paste0(z[1],"_",z[2]) })]
GO_IPI_pairs = GO_IPI_pairs[,.(ida, idb, pair_id, ida_clean, idb_clean, pair_id_clean, pubid, taxon, GO_IPI)]
# this table includes all taxons
...
The GO IPI dataset contains `r N_GO_IPI` interacting pairs. 

All tables in the results folder are filtered for human data only. Venn Diagrams show human data only.
Saving the table of interacting pairs, publication IDs and GO IPI tag.
```{r saving_results}
fwrite(x = unique(GO_IPI_pairs[taxon == "taxon:9606", .(pair_id_clean, pubid, GO_IPI)]), 
...
N_pubid_imex = length(imex[taxid_a == "9606" | taxid_b == "9606", unique(pubid)])
N_pubid_GO_IPI = length(GO_IPI_pairs[taxon == "taxon:9606",unique(pubid)])
N_pubid_overlap = sum(!is.na(match(GO_IPI_pairs[taxon == "taxon:9606",unique(pubid)], imex[,unique(pubid)])))
...
#### Compare GO IPI interactions and publications to IMEx 
...
N_imex = length(imex[taxid_a == "9606" | taxid_b == "9606",unique(pair_id_clean)])
N_GO_IPI = length(GO_IPI_pairs[taxon == "taxon:9606",unique(pair_id_clean)])
N_overlap = sum(!is.na(match(GO_IPI_pairs[taxon == "taxon:9606",unique(pair_id_clean)], imex[taxid_a == "9606" | taxid_b == "9606", unique(pair_id_clean)])))
...
N_pub_int_imex = length(imex[taxid_a == "9606" | taxid_b == "9606", unique(paste0(pubid,"_",pair_id_clean))])
N_pub_int_GO_IPI = length(GO_IPI_pairs[,unique(paste0(pubid,"_",pair_id_clean))])
N_pub_int_overlap = sum(!is.na(match(GO_IPI_pairs[,unique(paste0(pubid,"_",pair_id_clean))], imex[taxid_a == "9606" | taxid_b == "9606", unique(paste0(pubid,"_",pair_id_clean))])))
