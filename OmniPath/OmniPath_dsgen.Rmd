...
interactions[, pair_id := apply(data.table(source,target,stringsAsFactors = F), 1,
                                               function(a) { z = sort(a)
                                               paste0(z[1],"_",z[2]) })]
# extracting references
interactions = interactions[, .(source, target, sources, unlist(strsplit(references, ";"))), by = pair_id]
setnames(interactions, colnames(interactions)[5],"pubid")
interactions = unique(interactions)
nrow(interactions)
```

The total number of interacting pair in the interaction dataset: `r length(unique(interactions$pair_id))`

The total number of articles where evidence comes from: `r length(unique(interactions$references))`  
